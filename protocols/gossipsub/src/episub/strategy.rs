...
pub struct DefaultStrat {
    /// The maximum number of peers to return for each invocation of `choke_peers`.
    /// Default value is 2.
    choke_churn: u8,
    ...
    /// The maximum number of peers to return that are in the mesh and currently choked. The default value is 2.
    unchoke_churn: u8,
    ...
    /// pass the fanout_addition_strategy.
    fanout_churn: u8,
...
impl ChokingStrategy for DefaultStrat {
        ...
        let mut potential_choked_peers: HashSet<PeerId> = match self.choke_strategy {
            ChokeStrategy::RawLatencyCutoff(cutoff) => {
                ...
                metrics
                    ...
                    .filter_map(|(peer_id, stat)| {
                        if stat.latency >= cutoff && choked_mesh_peers.contains(&peer_id) {
                            Some(*peer_id)
                        } else {
                            None
                        }
                ...
                metrics
                    ...
                    .filter_map(|(peer_id, message_percent)| {
                        if *message_percent >= message_threshold
                            && choked_mesh_peers.contains(&peer_id)
                        {
                            Some(*peer_id)
                        } else {
                            None
            ...
            ChokeStrategy::LatencyOrderCutoff(cutoff) => {
                ...
                metrics
                    ...
                    .iter()
                    .filter_map(|(peer_id, stat)| {
                        if stat.order as u8 >= cutoff && choked_mesh_peers.contains(&peer_id) {
                            Some(*peer_id)
                        } else {
                            None
                        }
    ...
    fn unchoke_peers(
        ...
        let mut unchoke_peers = HashSet::new();
        ...
        match self.unchoke_strategy {
            UnchokeStrategy::IHaveMessagePercent(percent) => {
                ...
                for (peer_id, message_percent) in ihave_message_stats.iter() {
                    if *message_percent >= percent
                        && mesh_peers
                            .iter()
                            .find(|(find_peer_id, choke_state)| {
                                find_peer_id == peer_id && choke_state.peer_is_choked
                            })
                            .is_some()
                    {
                        if unchoke_peers.insert(*peer_id) {
                            inserted_count += 1;
            ...
            UnchokeStrategy::IHaveMessagePercent(percent) => {
                ...
                for (peer_id, message_percent) in ihave_message_stats.iter() {
                    if *message_percent >= percent
                        && mesh_peers
                            .iter()
                            .find(|(find_peer_id, _choke_state)| find_peer_id == peer_id)
                            .is_none()
                    {
                        if unchoke_peers.insert(*peer_id) {
                            inserted_count += 1;
